...
from src.models.cogview4 import CogView4Config, CogView4Model, convert_to_original_key
...
class CogView4ForTextToImageTraining(ModelForTraining, nn.Module):
    model: CogView4Model

    model_config: CogView4Config
    model_config_class = CogView4Config

    def setup_model(self):
        if self.accelerator.is_main_process:
            with init_empty_weights():
                self.model = CogView4Model(self.model_config)
    ...
    def get_state_dict_to_save(
        ...
        state_dict = {convert_to_original_key(k): v for k, v in state_dict.items()}
        return state_dict
...
def main(config: str):
    ...
    trainer.register_model_class(CogView4ForTextToImageTraining)
